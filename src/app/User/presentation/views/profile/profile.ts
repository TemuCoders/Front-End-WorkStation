...
export class Profile {
  ...
  readonly myWorkspaces = computed<Workspace[]>(() => {
    ...
    const owned = list.filter(w => {
      ...
      const ownerFull = owner ? `${owner.firstName ?? ''} ${owner.lastName ?? ''}`.trim().toLowerCase() : '';
  ...
  readonly reviews = signal<ReviewItem[]>([
    {
      id: 1, author: 'Liam Carter', date: 'June 2023', rating: 5,
      text: "Sophia's studio was perfect for my needs. It was clean, well-equipped, and in a great location. I highly recommend it!",
      likes: 2, comments: 0, avatar: 'https://i.pravatar.cc/48?img=12'
    },
    {
      id: 2, author: 'Emily Stone', date: 'May 2023', rating: 4,
      text: 'Great workspace and host. Internet was very fast and the room was quiet.',
      likes: 4, comments: 1, avatar: 'https://i.pravatar.cc/48?img=32'
    }
  ]);
  ...
  firstName(full: string): string { return (full || '').split(' ')[0] || ''; }
  avatarUrl(u: User): string { return u.photo || `https://i.pravatar.cc/120?u=${encodeURIComponent(u.email || String(u.id))}`; }
